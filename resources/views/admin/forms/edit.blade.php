...
<div class="px-4 sm:px-0 max-w-3xl">
    <div class="mb-6">
        <a href="{{ route('admin.forms.show', $form) }}" class="text-sm text-indigo-600 hover:text-indigo-500 focus-visible:ring-2 focus-visible:ring-indigo-500 rounded">
            ← Retour au formulaire
        </a>
        <h1 class="text-xl sm:text-2xl font-bold text-gray-900 mt-2">Modifier le formulaire</h1>
    </div>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                              class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:border-indigo-500 transition-colors duration-200">{{ old('description', $form->description) }}</textarea>
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="datetime-local" name="open_date" id="open_date"
                           class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:border-indigo-500 transition-colors duration-200"
                           value="{{ old('open_date', $form->open_date ? $form->open_date->format('Y-m-d\TH:i') : '') }}">
                </div>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="close_date" id="close_date"
                           class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:border-indigo-500 transition-colors duration-200"
                           value="{{ old('close_date', $form->close_date ? $form->close_date->format('Y-m-d\TH:i') : '') }}">
                </div>
                ...
                <div class="flex items-center">
                    <input type="checkbox" name="is_anonymous" id="is_anonymous" value="1"
                           ...
                           {{ old('is_anonymous', $form->is_anonymous) ? 'checked' : '' }}>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 sm:p-6">
            ...
            <div id="fields-container" class="space-y-4">
                @foreach($form->fields as $index => $field)
                    @include('admin.forms._field-row', ['field' => $field, 'index' => $index])
                @endforeach
            </div>
        </div>
