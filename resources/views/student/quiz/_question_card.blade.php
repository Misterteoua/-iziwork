{{--
    La carte de la question, telle que la voit le candidat.

    Un seul fragment pour deux usages : la page complète (student.quiz.question)
    et la réponse JSON qui remplace la carte sans recharger la page. Sans cette
    source unique, les deux affichages divergeraient — et c'est justement le
    moment où l'on découvre qu'un champ manque côté mobile.
...
<div class="bg-white rounded-2xl shadow-card border border-slate-200/70 p-6 sm:p-8">
    ...
    <form method="POST" action="{{ route('quiz.answer', $quiz->token) }}" class="mt-6 space-y-3" data-quiz-answer-form>
        ...
        @else
        {{-- La variable de boucle ne doit pas s'appeler $position : elle
             écraserait la place de la question, et le libellé du bouton
             annoncerait « passer à la suivante » sur la dernière question. --}}
        ...
        <label class="flex items-start gap-3 p-4 rounded-xl border border-slate-200 hover:border-brand-300 hover:bg-brand-50/40 cursor-pointer transition-colors duration-150">
            ...
            <span class="text-sm text-slate-800">
                ...
                <span class="font-semibold text-slate-500 mr-1">{{ chr(65 + $optionPosition) }}.</span> {{ $option['label'] }}
            </span>
        </label>
        @endforeach
        @endif

        {{-- Erreurs du rendu classique (sans JavaScript) --}}
        ...
        {{-- Erreurs de la réponse JSON : le fragment est réinséré tel quel, la
             zone reste donc identique dans les deux parcours. --}}
        <p data-quiz-error class="hidden text-sm text-red-600" role="alert"></p>

        <button type="submit"
                class="w-full inline-flex items-center justify-center px-5 py-3 border border-transparent text-sm font-semibold rounded-xl text-white gradient-bg hover:opacity-95 transition-all duration-150 disabled:opacity-60">
